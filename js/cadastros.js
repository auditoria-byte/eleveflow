...
function preencherSelectZonaEdificio(setor, zonaSelecionada){
  const sel = document.getElementById('edifZona');
  sel.innerHTML = '<option value="">Sem zona definida</option>';
  const zonasDoSetor = zonas.filter(z => z.setor === setor);
  zonasDoSetor.forEach(z => {
    const usuario = usuarios.find(u => u.id === z.usuarioId);
    const label = usuario ? `${z.nome} - ${usuario.nome}` : z.nome;
...
function abrirModalEdificio(id){
  ...
  document.getElementById('edifSetor').value = e.setor || '';
  ...
  preencherSelectZonaEdificio(e.setor, e.zona);
  document.getElementById('modalEditarEdificio').classList.add('ativo');
}
document.getElementById('edifSetor').addEventListener('change', function(){
  preencherSelectZonaEdificio(this.value, '');
});
